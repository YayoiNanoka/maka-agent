  ...
  switch (event.type) {
    ...
    case 'permission_request':
      return {
        ...
        author: 'system',
        actions: {
          permissionRequest: event.kind === 'additional_permissions'
            ? {
                ...
                entryCount: event.additionalPermissions?.fileSystem?.entries.length ?? 0,
                readCount: event.additionalPermissions?.fileSystem?.entries.filter((entry) => entry.access === 'read').length ?? 0,
                writeCount: event.additionalPermissions?.fileSystem?.entries.filter((entry) => entry.access === 'write').length ?? 0,
                exactCount: event.additionalPermissions?.fileSystem?.entries.filter((entry) => entry.scope === 'exact').length ?? 0,
                subtreeCount: event.additionalPermissions?.fileSystem?.entries.filter((entry) => entry.scope === 'subtree').length ?? 0,
                networkEnabled: (event.risk as import('@maka/core/permission').AdditionalPermissionRiskSummary | undefined)?.networkEnabled ?? false,
                outsideWorkspace: (event.risk as import('@maka/core/permission').AdditionalPermissionRiskSummary | undefined)?.outsideWorkspace ?? false,
                protectedMetadata: (event.risk as import('@maka/core/permission').AdditionalPermissionRiskSummary | undefined)?.protectedMetadata ?? false,
                alsoApprovesToolExecution: event.alsoApprovesToolExecution!,
                ...(event.hint !== undefined ? { hint: event.hint } : {}),
              }
            : event.kind === 'sandbox_escalation'
              ? {
                  ...
                  risk: event.risk as import('@maka/core/permission').SandboxEscalationRiskSummary,
                  alsoApprovesToolExecution: event.alsoApprovesToolExecution!,
                  ...(event.hint !== undefined ? { hint: event.hint } : {}),
                }
              : {
                ...
                toolName: event.toolName,
                category: event.category,
                reason: event.reason as import('@maka/core/permission').ToolPermissionRequest['reason'],
                args: event.args,
                ...(event.hint !== undefined ? { hint: event.hint } : {}),
              },
        },
    ...
    case 'permission_decision_ack':
      return {
        ...base,
        role: 'system',
        author: event.reviewer === 'auto_review' ? 'system' : 'user',
        actions: {
          permissionDecision: {
            requestId: event.requestId,
