...
import type { AdditionalPermissionRiskSummary, PermissionMode, ToolCategory } from '@maka/core/permission';
...
export function freezeAdditionalPermissionGrant(
  ...
  const profile = freezeAdditionalPermissionProfile(grant.profile);
  const normalizedPaths = freezeNormalizedPaths(grant.normalizedPaths);
...
export function assertAdditionalPermissionProposal(input: {
  ...
  const expectedIntentHash = stableHash({ toolName: input.toolName, args: input.args });
  if (
    !validated.ok
  ...
  for (const entry of profileEntries) {
    ...
    if (
      !normalized
      || !normalized.displayPath.startsWith('/')
...
export function resolveAdditionalPermissionCandidate(
  cwd: string,
  path: string,
  permissionContext: ToolExecutionPermissionContext | undefined,
): string {
  const lexical = resolve(cwd, path);
  for (const approved of permissionContext?.additionalGrant?.normalizedPaths ?? []) {
    const rel = relative(approved.displayPath, lexical);
    if (approved.scope === 'exact') {
      if (rel === '') return approved.enforcementPath;
      continue;
    }
    if (rel === '' || (!rel.startsWith('..') && !isAbsolute(rel))) {
      return resolve(approved.enforcementPath, rel);
    }
  }
  return lexical;
}
...
export async function normalizeAdditionalPermissionProfile(input: {
  ...
  const candidate = {
    ...(normalizedPaths.length > 0
      ? { fileSystem: { entries: normalizedPaths.map((entry) => ({
...
export async function revalidateAdditionalPermissionProposal(input: {
  ...
  const current = await normalizeAdditionalPermissionProfile({
    profile: {
      ...(input.proposal.normalizedPaths.length > 0 ? {
        fileSystem: { entries: input.proposal.normalizedPaths.map((entry) => ({
  ...
  if (serializeAdditionalPermissionProfile(current.profile) !== serializeAdditionalPermissionProfile(input.proposal.profile)) {
    throw new AdditionalPermissionError({
      stage: 'validation',
      reason: 'grant_path_changed',
      message: 'An approved additional permission path changed before execution.',
    });
  }
  ...
  if (input.proposal.normalizedPaths.some((approved) => (
    ...
    throw new AdditionalPermissionError({
      stage: 'validation',
      reason: 'grant_path_changed',
      message: 'An approved additional permission target changed type before execution.',
    });
  }
}
...
export async function planDeclaredBashAdditionalPermission(input: {
  ...
  if (!isRecord(input.declaration) || hasUnexpectedKeys(input.declaration, ['mode', 'file_system', 'network', 'justification'])) {
    return blockInvalid('Bash sandbox_permissions declaration is invalid.');
  }
  if (input.declaration.mode === 'use_default') {
    if (input.declaration.file_system !== undefined || input.declaration.network !== undefined || input.declaration.justification !== undefined) {
  ...
  try {
    normalized = await normalizeAdditionalPermissionProfile({
      profile: {
        ...(input.declaration.file_system !== undefined ? { fileSystem: input.declaration.file_system } : {}),
  ...
  const networkNeeded = Boolean(normalized.profile.network?.enabled)
    && !(input.context.profile.type === 'managed' && input.context.profile.network.kind === 'enabled');
  ...
  const neededPathKeys = new Set(neededEntries.map((entry) => `${entry.access}:${entry.scope}:${entry.path}`));
  const requiredPaths = normalized.normalizedPaths.filter((entry) => (
    neededPathKeys.has(`${entry.access}:${entry.scope}:${entry.enforcementPath}`)
  ));
...
function freezeAdditionalPermissionProfile(
  ...
  return Object.freeze({
    ...(entries.length > 0 ? { fileSystem: Object.freeze({ entries: Object.freeze(entries) }) } : {}),
...
function parseRawEntries(input: unknown): AdditionalFileSystemPermission[] {
  ...
  if (input.entries.length > MAX_ADDITIONAL_FILESYSTEM_ENTRIES) {
    throw invalidProfile(`Additional filesystem permissions are limited to ${MAX_ADDITIONAL_FILESYSTEM_ENTRIES} entries.`);
  }
