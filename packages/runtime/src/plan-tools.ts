...
> {
  return {
    name: 'update_plan',
    description:
      "Start or update the main Agent's current internal execution plan for complex work expected to finish in the current turn. Use it after initial reconnaissance when the task has three or more dependent stages, spans multiple components, requires investigation followed by implementation or verification, or has intermediate results that determine later work. Do not use task_create or task_update as a substitute for these ordered execution steps. Skip simple or single-step work. Submit the complete plan snapshot, keep at most one step in_progress, and continue execution immediately without waiting for user approval.",
    parameters: z.object({
