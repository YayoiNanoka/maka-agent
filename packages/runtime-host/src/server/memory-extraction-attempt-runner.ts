...
export const MEMORY_EXTRACTION_USER_INSTRUCTION = [
  ...
  '- Call memory_submit with action=propose exactly once. If it returns candidate cases, resolve every case in one action=resolve call. Treat same as duplicate, revises as a replacement of the same claim, distinct as independently true, and uncertain as not safely decidable.',
