...
test('an Origin lease remembers A→B→A instead of trusting the final URL', () => {
  let url = 'https://a.example/start';
  const tracker = new BrowserOriginLeaseTracker(() => url);
  const lease = tracker.open(url, 'observe');
  url = 'https://b.example/private?token=secret';
  tracker.recordNavigation(url);
  url = 'https://a.example/back';
  tracker.recordNavigation(url);

  assert.deepEqual(lease.snapshot(), {
    epoch: 2,
    url: 'https://a.example/back',
    violatedUrl: 'https://b.example/private?token=secret',
  });
});
